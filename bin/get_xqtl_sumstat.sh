...
if [ -s ${geno_dir}/${line}.vcf.gz ] ; then
	...
	${BGW_dir}/bin/Estep_mcmc -vcf ${geno_dir}/${line}.vcf.gz -p ${gene_trait} -maf 0.01 -miss ${miss} -o ${line} -LDwindow ${LDwindow} -GTfield ${GTfield} -target_chr ${target_chr} -start_pos ${start_pos} -end_pos ${end_pos} -saveSS -zipSS
