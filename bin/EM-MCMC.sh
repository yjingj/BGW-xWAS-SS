...
################################################################
################################################################
# Training BGW-xWAS molecular trait prediction model by EM-MCMC algorithm
...
burnin=${burnin:-10000}
Nmcmc=${Nmcmc:-10000}
CPP_thresh=${CPP_thresh:-0.001}
num_cores=${num_cores:-1}
pp_cis=${pp_cis:-0.0003}
pp_trans=${pp_trans:-0.00001}
a_gamma=${a_gamma:-1}
b_gamma=${b_gamma:-2}
...
if [ -s ${wkdir}/${gene_name}_EM_MCMC/Eoutput/paramtemp${em}.txt.gz ] ; then
    echo -e "#CHR\tPOS\tID\tREF\tALT\tMAF\tTrans\tCPP\tBeta\tmBeta\tChisqTest\tPval_svt\tRank" > ${wkdir}/${gene_name}_BGW_xQTL_weights.txt
    zcat ${wkdir}/${gene_name}_EM_MCMC/Eoutput/paramtemp${em}.txt.gz | tail -n+2 | awk -v CPP_thresh=${CPP_thresh} '$8>CPP_thresh{print }'  | sort -nk1,2 >> ${wkdir}/${gene_name}_BGW_xQTL_weights.txt
else
    echo File ${wkdir}/${gene_name}_EM_MCMC/Eoutput/paramtemp${em}.txt.gz is not generated by EM-MCMC algorithm. Please Check.
    exit
fi

if [ $clean_output -eq 1  ]; then
    rm -rf ${wkdir}/${gene_name}_EM_MCMC
    rm -f ${wkdir}/${gene_name}_trait.txt
fi
