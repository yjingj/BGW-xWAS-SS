...
if [ -s ${wkdir}/${gene_name}_EM_MCMC/Eoutput/paramtemp${em}.txt.gz ] ; then
    echo -e "#CHR\tPOS\tID\tREF\tALT\tMAF\tTrans\tPCP\tBeta\tmBeta\tChisqTest\tPval_svt\tRank" > ${wkdir}/${gene_name}_BGW_eQTL_weights.txt
    zcat ${wkdir}/${gene_name}_EM_MCMC/Eoutput/paramtemp${em}.txt.gz | tail -n+2 | awk -v PCP_thresh=${PCP_thresh} '$8>PCP_thresh{print }'  | sort -nk1,2 >> ${wkdir}/${gene_name}_BGW_eQTL_weights.txt
else
